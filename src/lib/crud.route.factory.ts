...
export class CrudRouteFactory {
    ...
    protected destroy<T>(controllerMethodName: string): void {
        this.targetPrototype[controllerMethodName] = function reservedDestroy(crudDeleteOneRequest: CrudDeleteOneRequest<T>) {
            return this.crudService.reservedDestroy(crudDeleteOneRequest);
        };
    }
    ...
    private createMethod(crudMethod: Method): void {
        if (crudMethod === Method.RECOVER) {
            const enableRecover = this.crudOptions.routes?.[Method.DESTROY]?.softDelete ?? CRUD_POLICY[Method.DESTROY].default.softDeleted;
            if (!enableRecover) {
                return;
            }
        }
        ...
        const needPagination = crudMethod === Method.INDEX || crudMethod === Method.SEARCH;
    ...
    private defineParameterSwagger(method: Method, params: string[], target: Object, paginationType?: PaginationType) {
        ...
        if (method === Method.INDEX && paginationType) {

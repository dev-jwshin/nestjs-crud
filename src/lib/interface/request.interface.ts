...
export type CrudCreateRequestDto<T, AllowedKeys extends keyof T = keyof T> =
    Partial<CrudAllowedParams<T, AllowedKeys>>;

export type CrudUpdateRequestDto<T, AllowedKeys extends keyof T = keyof T> =
    Partial<CrudAllowedParams<T, AllowedKeys>>;

export type CrudUpsertRequestDto<T, AllowedKeys extends keyof T = keyof T> =
    Partial<CrudAllowedParams<T, AllowedKeys>>;

// Helper to create type-safe DTOs with literal array of allowed params
export type CreateCrudCreateRequestDto<T, AllowedParams extends readonly (keyof T)[]> =
    Pick<T, AllowedParams[number]>;

export type CreateCrudUpdateRequestDto<T, AllowedParams extends readonly (keyof T)[]> =
    Partial<Pick<T, AllowedParams[number]>>;

export type CreateCrudUpsertRequestDto<T, AllowedParams extends readonly (keyof T)[]> =
    Pick<T, AllowedParams[number]>;

// Runtime helper functions to filter request data based on allowedParams
export function filterAllowedCreateParams<T, K extends keyof T>(
    data: any,
    allowedParams: K[]
): Pick<T, K> {
...
export function filterAllowedUpdateParams<T, K extends keyof T>(
    data: any,
    allowedParams: K[]
): Partial<Pick<T, K>> {
    if (!data || typeof data !== 'object') {
...
export function validateAllowedParams<T>(
    data: any,
